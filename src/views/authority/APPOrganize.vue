...
export default class InterfaceManagement extends Vue {
  ...
  addRules = {
    ...
    code: { required: true, trigger: "blur", message: "请输入接口" }
  };
  ...
  handleAddSubmit() {
    ...
    rest.validate((valid: any) => {
      if (valid) {
        ...
        let url = this.addForm.id ? Url.auth.updateAPPOrganize : Url.auth.addAPPOrganize;
  ...
  handleDelete(item: any) {
    ...
    this.$confirm("确认删除？", "提示", {
      ...
      .then(() => {
        Http.get(Url.auth.deleteAPPOrganize+"/"+item.id).then((res: any) => {
          if (res.status) {
            if (res.data.status) {
              this.$message({
                type: "success",
